...
def runge_kutta(t, q1, q2, q1_dot, q2_dot, action, dt):
    k1 = dt * update_world(q1, q2, q1_dot, q2_dot, action)
    k2 = dt * update_world(q1 + 0.5 * k1[0], q2 + 0.5 * k1[1], q1_dot + 0.5 * k1[2], q2_dot + 0.5 * k1[3], action)
    k3 = dt * update_world(q1 + 0.5 * k2[0], q2 + 0.5 * k2[1], q1_dot + 0.5 * k2[2], q2_dot + 0.5 * k2[3], action)
    k4 = dt * update_world(q1 + k3[0], q2 + k3[1], q1_dot + k3[2], q2_dot + k3[3], action)
...
def digitize_state(q1, q2, q1_dot, q2_dot):
    ...
    q1_dot_bin = np.digitize(q1_dot, np.linspace(-10.0, 10.0, num_q1_dot_bins + 1)) - 1
    q2_dot_bin = np.digitize(q2_dot, np.linspace(-10.0, 10.0, num_q2_dot_bins + 1)) - 1
...
def q_learning(runge_kutta):
    for epoch in range(num_episodes):  
        ...
        # CSVファイルの準備
        csv_file_path = f'try_{epoch + 1}.csv'
        with open(csv_file_path, 'w', newline='') as csvfile:
            ...
            for i in range(max_number_of_steps):
                ...
                q1_bin, q2_bin, q1_dot_bin, q2_dot_bin = digitize_state(q1, q2, q1_dot, q2_dot)
                print(f'theta1: {q1 * np.pi / 180}, theta2: {q2 * np.pi / 180}, omega1: {q1_dot}, omega2: {q2_dot}')
