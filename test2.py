...
# 報酬関数
def calculate_reward(q1, q2, q1_dot, q2_dot, previous_q1, previous_q2):
    # q2が可動範囲を超える場合
    if q2 < 0 or q2 > 29 * np.pi / 36:
        reward = -10
    else:
        # 前のステップとの比較
        if q1 > previous_q1:  # 反時計回りに移動した場合
            reward = 1
        elif q1 < previous_q1:  # 時計回りに移動した場合
            reward = -1
        else:
            reward = 0

        # 1周した場合の報酬
        if q1 > 0 and previous_q1 < 0:  # 1周した場合
            reward += 50000
...
def q_learning(update_world):
    for epoch in range(100):  
        ...
        # CSVファイルの準備
        csv_file_path = f'dp_euler2_epoch_{epoch + 1}.csv'
        with open(csv_file_path, 'w', newline='') as csvfile:
            ...
            for i in range(6000):
                ...
                # 報酬の計算
                reward = calculate_reward(q1, q2, q1_dot, q2_dot, previous_q1, previous_q2)
                total_reward += reward
                ...
                # 1周した場合に学習を終了し、次のエピソードへ
                if reward >= 30000:
                    print(f"Completed one full rotation in epoch {epoch + 1} at step {i + 1}. Moving to next episode.")
                    break
